...
- `altitude_data_chan` — Calibrated altitude in meters (derived from pressure, published by Altimeter app)
...
# IMPORTANT: CREATE HELPER SCRIPTS FOR REPETITIVE TASKS
Repetitive tasks that can be handled by a script you can create and place in .agent_tools
Here are already some scripts you can use and edit however you like. See this as your AI playground, nothing that will be comitted, hack as much as you'd like.

# IMPORTANT: HOW TO TEST
READ: .github/instructions/agent-testing.instructions.md
NEVER USE FILE WRITE OPERARTIONS LIKE  File write operations detected: /dev/null. This stops the agent. Use python scripts in .agent_tools for any helper scripts you need to create. You can also run scripts directly from the terminal if needed.

## Testing with pytest (preferred over manual scripts)

For testing firmware on native_sim or hardware, use the pytest tests in `app/pytest/`:

```bash
source .venv/bin/activate
cd app/pytest

# native_sim: boot, launch app, screenshot, close
pytest test_native_app.py::TestNativeSim -s --app "Calculator"

# Hardware: launch app, shell commands
pytest test_hw_app.py::test_app_launches -s --board watchdk@1 --skip-flash --app "Calculator"
pytest test_hw_app.py::test_shell_cmd -s --board watchdk@1 --skip-flash --shell-cmd "battery"
```