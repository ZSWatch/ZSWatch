...
static struct stream_flash_ctx full_fs_stream_ctx;
static uint8_t full_fs_stream_buf[512] __aligned(4);
static bool full_fs_stream_active;
...
static int zsw_fs_open(struct fs_file_t *zfp, const char *file_name, fs_mode_t mode)
{
    ...
    if (IS_SPECIAL_FULL_FS_FILE_PATH(file_name_ptr)) {
        ...
        if (mode & FS_O_WRITE) {
            ...
            int rc = stream_flash_init(&full_fs_stream_ctx, flash_dev,
                                       full_fs_stream_buf, sizeof(full_fs_stream_buf),
                                       flash_area->fa_off, flash_area->fa_size, NULL);
            if (rc != 0) {
...
static int zsw_fs_close(struct fs_file_t *zfp)
{
    if (IS_SPECIAL_FULL_FS_FILE(zfp->filep)) {
        ...
        if (full_fs_stream_active) {
            rc = stream_flash_buffered_write(&full_fs_stream_ctx, NULL, 0, true);
            if (rc != 0) {
                LOG_ERR("stream_flash flush failed: %d", rc);
            } else {
                full_fs_file.len = stream_flash_bytes_written(&full_fs_stream_ctx);
            }
...
static int zsw_fs_seek(struct fs_file_t *zfp, off_t offset, int whence)
{
    ...
    if (IS_SPECIAL_FULL_FS_FILE(zfp->filep)) {
        if (full_fs_stream_active) {
            LOG_ERR("Seek not supported while streaming full_fs writes");
            return -ENOTSUP;
        }
        switch (whence) {
            case FS_SEEK_SET:
