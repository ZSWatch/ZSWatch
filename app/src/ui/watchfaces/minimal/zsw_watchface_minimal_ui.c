...
static void watchface_show(lv_obj_t *parent, watchface_app_evt_listener evt_cb, zsw_settings_watchface_t *settings)
{
    ...
    lv_obj_set_style_bg_img_src(ui_minimal_watchface, watchface_app_get_bg(), LV_PART_MAIN | LV_STATE_DEFAULT);
    ...
    if (settings->animations_on) {
        ...
        lv_gif_set_src(img, ZSW_LV_IMG_USE_WITH_MOUNT(snoopy_alt, "/lvgl_lfs"));
...
{
    if (ui_minimal_watchface && lv_obj_is_valid(ui_minimal_watchface)) {
        lv_obj_set_style_bg_img_src(ui_minimal_watchface, bg_img, LV_PART_MAIN | LV_STATE_DEFAULT);
    }
}
