...
static zsw_app_category_t current_folder = INVALID_FOLDER_ID;
...
static void row_focused(lv_event_t *e)
{
    ...
    if (row && lv_obj_get_child_cnt(row) > 0 && !is_deleting_app_picker) {
        ...
        if (current_folder == INVALID_FOLDER_ID) {
...
static void folder_back_clicked(lv_event_t *e)
{
    current_folder = INVALID_FOLDER_ID;
    delete_root_object();
...
static void draw_apps_in_folder(zsw_app_category_t category)
{
    ...
    // Add back button
    entry = create_application_list_entry(grid, ZSW_LV_IMG_USE(close_icon), "Back", -1, false);
