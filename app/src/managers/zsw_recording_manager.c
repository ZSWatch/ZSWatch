...
#define CODEC_THREAD_STACK     16384
...
/* Ring buffer for PCM data from mic to codec thread */
...
int zsw_recording_manager_start(void)
{
    ...
    if (ret < 0) {
        ...
        zsw_recording_manager_store_stop_recording(NULL, NULL);
        return ret;
    }
...
int zsw_recording_manager_stop(void)
{
    ...
    zsw_recording_manager_store_stop_recording(&duration_ms, &size_bytes);
