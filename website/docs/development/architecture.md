...
At startup, `zsw_clock_init()` probes the RTC device. If the RTC is not responding or fails to accept time updates, the fallback is automatic and transparent. A common cause of RTC unavailability is **no battery connected** — without a battery the RTC loses power and fails to respond. Apps can query RTC availability at runtime:
